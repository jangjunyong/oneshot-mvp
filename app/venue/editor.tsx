...
export default function Editor({
  ...
  return (
    <div className="venue-layout">
      ...
      <p className="note">
        기호 읽는 법. 해칭은 무대, 점은 주차장, 이중 테두리는 화장실, 굵은
        양끝은 출입구, 점선 띠는 통로입니다. 도형을 누르면 치수가 붙습니다.
      </p>
      </div>
      ...
      <aside className="venue-panel">
        ...
        {mode === "site" && (
          <p className="note">
            부지의 모서리를 차례로 눌러 찍고 경계 닫기를 누르세요. 세 점 이상이면
            됩니다. 부지가 있으면 면적이 나오고, 밖으로 나간 배치를 잡아냅니다.
            닫고 나서 &ldquo;부지에 맞춰 보기&rdquo;를 누르면 그 부지가 화면에 꽉
            차서 3m 부스도 손으로 잡을 수 있습니다
          </p>
        )}
        {site && (
          <>
          ...
          <>
            ...
            <p className="note">
              ...
              {(venue.map?.zoom ?? 0) >= 줌범위.max && site && (
                <>
                  {" "}확대가 여기서 막히면 <strong>부지에 맞춰 보기</strong>를 누르세요.
                  있는 타일을 늘려 그리는 것이라 배경은 흐려지지만 치수는
                  그대로입니다
                </>
              )}
            </p>
        ...
        {!scenario ? (
          <p className="note">
            진단 이력에서 &ldquo;이 쏠림에 대비하기&rdquo;를 눌러 들어오면 닮은 축제의
            실측 배수로 부스를 전부 스캔합니다
          </p>
        ) : scan?.blocked ? (
          ...
          <>
            ...
            {scan && (
              <>
                ...
                <p className="note">
                  가정 세 가지를 밝힙니다. 대기열은 부하 1 초과분에 부스 깊이를 곱해
                  그립니다. 인력을 안 적은 부스는 0.5명으로 칩니다. 부하는 상대
                  지수일 뿐 방문객 수 예측이 아닙니다.
                </p>
              </>
            )}
          </>
