...
//
// 키를 받아 실제로 쟀다(NAVER API HUB 발급). **표본 크기를 바꿔 가며** 넷을
// 쟀다 — 한 번만 재고 정하면 그게 표본의 우연인지 신호인지 모른다.
//
//   표본 N   유효 n   피어슨 r   스피어만 ρ
//      60      51      0.457      0.243
//     120      94      0.515      0.340   <- 컷을 넘는 유일한 지점
//     200     156      0.084      0.221
//     300     237      0.155      0.236   <- 가장 큰 표본
//
// **버린다.** 컷(|ρ| 0.3)을 넘는 것은 넷 중 하나뿐이고, 가장 큰 표본이 0.236
// 이다. 피어슨이 0.084~0.515 로 널뛰는 것은 몇몇 이상치가 끌고 있다는 뜻이다
// (검색 32배인데 실측 2.47배 같은 축제). `season.ts` 의 `robust` 와 같은
// 논리다 — **표본을 바꾸면 뒤집히는 신호는 신호가 아니다.**
//
// 생존 편향도 봤다. 검색량이 0 이라 뺀 쪽의 중앙 배수는 1.26, 상관에 든 쪽은
// 1.28~1.29 로 거의 같다. 빠진 것이 특정 구간에 몰려 있지는 않다.
//
// ── 앞선 판정이 왜 틀렸었나 (같은 함정을 두 번 파지 말 것) ──
//
// 처음엔 r=0.037, ρ=0.192 로 재고 버렸다. **그 숫자는 표본이 잘못된 것이었다.**
// `evals/search-volume.mjs` 의 표본 추출이 배수 오름차순 정렬에서 뒤를 잘라
// 고배수 구간(2.27~2.77)을 통째로 빼먹고 있었다. 고친 뒤 같은 N 에서 ρ 가
// 0.192 -> 0.340 으로 뛰었다. 결론은 살아남았지만 **근거는 갈렸다.**
// 재는 방법이 틀리면 결론이 맞아도 맞은 게 아니다.
//
// 덤으로 사용자의 가설은 수치로 끝났다 — "타겟이 고령이면 검색량에 안 잡힌다":
// 60세 이상의 개최 전 배수가 전체보다 낮은 축제가 **145건 중 115건(79%)**.
// 네 번 다 76~80% 로 안정적이다. 방향은 맞았다. 다만 이 축을 살릴 근거는 아니다.
//
