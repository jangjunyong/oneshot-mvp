...
/**
 * 국토가 다 들어가는 범위. 축제 좌표(126.23~129.46)가 아니라 **해안선**이
 * 범위를 정한다 — 백령도(124.61)와 독도(131.86)를 지도 밖으로 밀어내면
 * 그건 남한 지도가 아니다.
 *
 * 축제 점만 있던 때는 이 폭이 빈 바다였고 그래서 한 번 좁혔었다.
 * 지금은 그 바다에 섬과 해안선이 있다.
 */
const LAT_MAX = 38.7;
const LAT_MIN = 33.15;
const LNG_MIN = 124.5;
...
/**
 * 지도 캔버스 크기. 위도 5.55° × 경도 7.5°이고 경도 1°는 위도 36°에서
 * cos 보정(≈0.809)만큼 짧다 → 가로:세로 = 7.5×0.809 : 5.55 ≈ 1.093.
 */
export const MAP_H = 470;
export const MAP_W = 514;
