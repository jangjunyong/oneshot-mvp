  ...
  if (cap?.ratio !== null && cap?.ratio !== undefined) {
    ...
    if (v.breakevenTurnover !== null) {
      seg.push(
        ...
        { t: "회 이상 와야 이 숫자가 된다(손익분기 회전율 = N ÷ (0.80 × 상한)). 회전율은 가정하지 않는다. " },
      );
    }
  }
...
/**
 * 보완 문장 — 판정마다 담당자가 다음에 할 일. 숫자는 슬롯·비(배)뿐이라 명 수를 새로 만들지 않는다.
 * "안전하다"는 말은 어느 라벨에도 없다.
 */
export function adviseVisitors(v: VisitorVerdict): Segment[] {
  const hist = v.historyMult;
  const unit = v.basis ? BASIS_KO[v.basis] : "";
  switch (v.label) {
    case "단위 미상":
    ...
    case "과대":
      return [
        { t: "기획안이 요구하는 배수" },
        ...(v.requiredMult !== null ? [{ t: "(" }, { ratio: v.requiredMult, digits: 2 }, { t: "배)" }] : []),
        { t: "는 이 축제 이력 최대" },
        ...(hist !== null ? [{ t: "(" }, { ratio: hist, digits: 2 }, { t: "배)" }] : []),
        { t: "의 1.3배를 넘는다. 이력이 두 해 이상 같은 자리였다면 내년도 그 구간 안일 가능성이 크다. 예상 방문객을 이력 배수 구간으로 내리고, 넘는 근거(신규 프로그램·매체 노출)가 있으면 그 근거를 별도 항목으로 적는다." },
      ];
    case "주의":
      return [
        { t: "기획안이 이력 최대 배수의 1.0~1.3배 사이거나, 순증 대비 비가 임계를 넘는다. 이력 상한을 기준으로 물량을 잡되 넘는 몫은 예비로 두고, 축제가 실제로 끌어온 순증" },
        ...(v.stages.find((s) => s.id === "increment")?.ratio !== null ? [{ t: "(비 " }, { ratio: v.stages.find((s) => s.id === "increment")!.ratio!, digits: 1 }, { t: ")" }] : []),
        { t: "을 예산 심의에 같이 낸다." },
      ];
    case "과소":
      return [
        { t: "기획안이 이력 최대 배수의 0.7배 아래다. 실제로 온 만큼을 감당 못 할 위험이 있다. 이력 배수 구간" },
        ...(hist !== null ? [{ t: "(최대 " }, { ratio: hist, digits: 2 }, { t: "배)" }] : []),
        { t: "으로 물량을 올려 잡는다." },
      ];
    case "통과":
    default:
      return [
        { t: "기획안이 이 축제 이력 배수 안에 있다. 안전하다는 뜻이 아니라 실측이 겪은 범위 안이라는 뜻이다. 물량은 이력 최대 배수" },
        ...(hist !== null ? [{ t: "(" }, { ratio: hist, digits: 2 }, { t: "배)" }] : []),
        { t: "를 상한으로 잡고, 반경 50km 같은 기간 경쟁과 연휴는 별도로 본다." },
      ];
  }
}
