...
function descend(grid, field, x, y) {
  // 내리막 방향 = 8이웃 중 더 낮은 셀들로의 단위벡터를 (낙차/거리) 로 가중해 더한 것.
  // 전에는 "가장 낮은 한 셀"을 골랐는데, 직선 통로에서는 정면과 정면 대각선의 값이 같아
  // 루프 순서(dy=-1 먼저)가 대각선을 뽑았다 → 벽까지 비스듬히 가서 벽 반발로 튕기는 지그재그.
  // 폭 2m 통로 실측 vx 0.69 / 속도 1.15 (fd_test, 2026-09-09) 가 그 증거다. 가중 합이면
  // 대각선 둘의 옆 성분이 상쇄돼 정면으로 가고, 굽은 곳에서는 부드럽게 꺾는다 (계획 10 S2).
  const [cx, cy] = grid.toCell(x, y);
  if (!grid.inside(cx, cy)) return [0, 0];
  const here = field[grid.idx(cx, cy)];
  let sx = 0, sy = 0, best = Infinity, bx = 0, by = 0;
  for (let dy = -1; dy <= 1; dy++) for (let dx = -1; dx <= 1; dx++) {
    ...
    sx += (dx / d) * (drop / d); sy += (dy / d) * (drop / d);
  }
  if (best === Infinity) return [0, 0];
  let l = Math.hypot(sx, sy);
  if (l < 1e-9) { // 여기가 국소 최저(목적지 안)거나 평지 — 가장 낮은 셀 쪽으로
    const [tx, ty] = grid.toWorld(cx + bx, cy + by);
    sx = tx - x; sy = ty - y; l = Math.hypot(sx, sy) || 1;
  }
  return [sx / l, sy / l];
}
...
export class Simulation {
  constructor(venue, scenario, opts = {}) {
    ...
    this.dt = opts.dt ?? 0.1;
    // 헤드웨이 시간 T(s). JuPedSim 기본 1.0 은 폭 2m 통로에서 Weidmann 보다 35% 느렸다(fd_test 2026-09-09).
    // 0.7 이면 7점 중 6점이 ±30% 안 (전부 -20~-30%, 느린 쪽 편향 — 밀도를 후하게 보는 방향)
    this.T = opts.T ?? 0.7;
    this.time = 0;
    this.agents = [];
    this.nextId = 1;
    ...
    this._neighbors = [];
    this._placeNb = [];
    this._placed = []; // 이번 스텝에 관람 자리로 옮겨 세운 사람
  }
  ...
  step() {
    ...
    for (const a of this.agents) {
      ...
      // 벽 반발 — 가장 가까운 벽 지점까지의 실제 거리·방향으로. 전에는 셀 중심 거리(wallDist×cell−cell/2)라 1m 격자에선 벽에 붙은
      // 셀도 0.5m 로 잡혀 힘이 5·e⁻⁵≈0 이었다. 그 상태로 벽 여유 거리 규칙만 넣으니 모서리로 곧장 가다 이동이 거절되기를 되풀이해
      // 폭 1m 틈이 막혔다(_diag_gap.mjs: 분당 통과 68→42→25, 틈 안 0명, 2026-09-14). 방향을 미리 틀어야 규칙에 걸리지 않는다
      const wn = wallNearest(this.grid, a.x, a.y, 2 * lMin);
      if (wn.d < 2 * lMin) {
        const [gx, gy] = wn.d > 1e-9 ? [wn.ux, wn.uy] : wallGradient(this.grid, this.wallDist, cx, cy);
        const w = kWall * Math.exp(-(wn.d - lMin / 2) / dWall);
        rx += gx * w; ry += gy * w;
      }
      ...
      // 몸 중심은 벽(걸을 수 없는 셀)에서 반지름 이상 떨어져야 한다. 전에는 중심점의 walkable 만 봐서 몸 반쪽이 부스 안에 든 채
      // 벽선에 줄지어 붙었고, 1m×2m 띠 칸에 15명(7.50명/㎡)이 들어갔다(_diag_density.mjs 101.80분, 2026-09-14). 벽 반발은 셀 중심
      // 거리라 1m 격자에선 벽에 붙은 셀도 0.5m 로 잡혀 이걸 막지 못한다. 이미 너무 붙은 사람이 벽에서 멀어지는 이동은 언제나 된다.
      const rWall = 0.2 * Math.sqrt(this.k), cNow = wallClearance(this.grid, a.x, a.y, rWall);
      const okAt = (x, y) => { const c = wallClearance(this.grid, x, y, rWall); return c >= rWall || c > cNow; };
      if (okAt(tx, ty)) { a.x = tx; a.y = ty; }
      else if (okAt(tx, a.y)) { a.x = tx; }
      else if (okAt(a.x, ty)) { a.y = ty; }
    }
    // 부스 서비스
    for (const at of this.venue.attractions) {
      ...
      if (at.front) {
        ...
        // 줄 자리가 못 걷는 곳(부지 밖·장애물)이면 전에는 가장 가까운 셀 중심으로 몰아 넣어 여럿이 한 점에
        // 겹쳤다 — 공원 경계에 붙은 부스에서 20명/2㎡ = 10명/㎡ (2026-09-09 실측). 이제는 줄 방향으로 0.5m 씩
        // 최대 6m 물러나며 걸을 수 있는 자리를 찾고, 그래도 없으면 옆으로 i 번째만큼 비켜 놓는다.
        const slot = (nx, ny, i) => {
          if (this.grid.walkableAt(nx, ny)) return [nx, ny];
          for (let j = 1; j <= 12; j++) {
            const px = nx + back[0] * 0.5 * j, py = ny + back[1] * 0.5 * j;
            if (this.grid.walkableAt(px, py)) return [px, py];
          }
          const p = nearestWalkable(this.grid, nx, ny);
          if (!p) return null;
          const jitter = 0.45 * ((i % 7) - 3);
          const qx = p[0] + side[0] * jitter, qy = p[1] + side[1] * jitter;
          return this.grid.walkableAt(qx, qy) ? [qx, qy] : p;
        };
        // 자리에 누가(몸 반지름 안에) 서 있으면 이번 스텝은 제자리에서 기다린다. 응대가 끝난 사람은 walk 로 바뀐 그 스텝엔
        // 아직 자리에 있는데, 줄 맨 앞 사람을 그 자리로 순간이동시켜 둘이 한 점에 겹쳤다 — 0.05m 안 쌍 첫 6건이 전부 이 교대였다
        // (_diag_overlap.mjs, 2026-09-14). 이웃은 스텝 첫머리 해시로 찾되 좌표는 지금 값을 읽으니 이번 루프에서 옮긴 사람도 잡힌다.
        const rBody = 0.5 * lMin;
        const place = (ag, nx, ny, i = 0) => {
          const s = slot(nx, ny, i);
          if (!s) return;
          for (const b of hash.near(s[0], s[1], nb)) if (b !== ag && Math.hypot(b.x - s[0], b.y - s[1]) < rBody) return;
          ag.x = s[0]; ag.y = s[1];
        };
        const nS = q.serving.length, gapS = Math.min(1.0, frontage / Math.max(1, nS));
        q.serving.forEach((ag, i) => {
          const lateral = (i - (nS - 1) / 2) * gapS;
          place(ag, at.front[0] + side[0] * lateral, at.front[1] + side[1] * lateral, i);
        });
        q.waiting.forEach((ag, i) => {
          const row = Math.floor(i / perRow), col = i % perRow;
          const lateral = (col - (perRow - 1) / 2) * step * (row % 2 === 0 ? 1 : -1);
          const along = (row + 1) * rowGap;   // 첫 줄은 서비스 줄 뒤 0.8m 부터
          place(ag, at.front[0] + back[0] * along + side[0] * lateral, at.front[1] + back[1] * along + side[1] * lateral, i);
        });
      }
    }
    // 퇴장
  ...
  arrive(a, attractionsById) {
    ...
    if (at.kind === "stage" || at.kind === "view") { // 무대·관람은 줄 없이 머문다 — 관람 구역(다각형) 안에 고르게 흩어져 선다
      ...
      // 뽑은 점에 이미 누가(몸 반지름 안에) 서 있으면 다시 뽑는다. 체류 중엔 움직이지 않아 겹침이 관람 시간 내내 남았다
      // (_diag_overlap.mjs 85.70분 관람석 3.6cm, 2026-09-14). 이번 스텝에 옮겨 세운 사람은 해시 칸이 옛 자리라 따로 본다
      const rBody = 0.2 * Math.sqrt(this.k);
      const taken = (x, y) => {
        for (const b of this.hash.near(x, y, this._placeNb)) if (b !== a && Math.hypot(b.x - x, b.y - y) < rBody) return true;
        for (const b of this._placed) if (b !== a && Math.hypot(b.x - x, b.y - y) < rBody) return true;
        return false;
      };
      let p = randomPointIn(area, this.rng, this.grid);
      for (let tries = 0; tries < 20 && taken(p[0], p[1]); tries++) p = randomPointIn(area, this.rng, this.grid);
      a.x = p[0]; a.y = p[1];
      this._placed.push(a);
      return;
    }
  ...
  updateDensity() {
    this.density.fill(0);
    const counts = this._counts ??= new Float32Array(this.dW * this.dH);
    counts.fill(0);
    for (const a of this.agents) {
      const cx = Math.floor((a.x - this.grid.minX) / this.dCell), cy = Math.floor((a.y - this.grid.minY) / this.dCell);
      if (cx >= 0 && cy >= 0 && cx < this.dW && cy < this.dH) counts[cy * this.dW + cx] += this.k;
    }
    binDensity(counts, this.dArea, this.dW, this.dH, this.dCell * this.dCell, this.cell * this.cell, this.density);
...
/**
 * 밀도(명/㎡) = 칸의 사람 수 ÷ 칸의 걸을 수 있는 면적. 순수 함수 — out 에 채워 돌려준다.
 *
 * 걸을 수 있는 면적이 칸의 절반 미만인 조각 칸(산책로·부스 가장자리)은 3×3 이웃과 합쳐 잰다.
 * 원인(2026-09-14, 시뮬_데모/_diag_density.mjs): 군포·기본 유입·점=1명으로 94분에 처음 7.22 를 넘은 칸은 걸을 수 있는 면적이
 * 1㎡(1m 셀 하나)뿐이었고, 안에 있던 8명은 서로 0.46m 넘게 떨어져 걷고 있었다 — 겹친 게 아니라 몸이 옆 칸에 걸친 사람들을 1㎡ 로 나눠 8.00.
 * 화면의 9.00 도 같은 조각 칸이다. 칸 절반 이상이 걸을 수 있으면 예전처럼 자기 면적으로만 잰다(도로 가장자리를 4㎡ 로 희석하지 않던 규칙).
 * 면적 0 칸도 합쳐 잰다 — 경계선 좌표에 선 사람이 밀도에서 사라지면 안 된다.
 */
export function binDensity(counts, areas, dW, dH, binArea, minArea, out) {
  for (let cy = 0; cy < dH; cy++) {
    for (let cx = 0; cx < dW; cx++) {
      const i = cy * dW + cx;
      if (areas[i] >= binArea / 2) { out[i] = counts[i] / Math.max(areas[i], minArea); continue; }
      let c = 0, a = 0;
      for (let y = Math.max(0, cy - 1); y <= Math.min(dH - 1, cy + 1); y++) {
        for (let x = Math.max(0, cx - 1); x <= Math.min(dW - 1, cx + 1); x++) { c += counts[y * dW + x]; a += areas[y * dW + x]; }
      }
      out[i] = c === 0 ? 0 : c / Math.max(a, minArea);
    }
  }
  return out;
}
...
/** 점에서 가장 가까운 걸을 수 없는 셀(격자 밖 포함)까지의 실제 거리 d(m)와, 그 벽 지점에서 점으로 향하는 단위벡터(ux, uy).
 *  reach 안에 없으면 d = Infinity. 점이 그런 셀 안이면 d = 0, 방향 0 */
function wallNearest(grid, x, y, reach) {
  const [cx, cy] = grid.toCell(x, y);
  const span = Math.ceil(reach / grid.cell);
  let d = Infinity, ux = 0, uy = 0;
  for (let yy = cy - span; yy <= cy + span; yy++) for (let xx = cx - span; xx <= cx + span; xx++) {
    if (grid.inside(xx, yy) && grid.walk[grid.idx(xx, yy)] === 1) continue;
    const x0 = grid.minX + xx * grid.cell, y0 = grid.minY + yy * grid.cell;
    const qx = Math.min(Math.max(x, x0), x0 + grid.cell), qy = Math.min(Math.max(y, y0), y0 + grid.cell);
    const dd = Math.hypot(x - qx, y - qy);
    if (dd < d) { d = dd; ux = dd > 1e-9 ? (x - qx) / dd : 0; uy = dd > 1e-9 ? (y - qy) / dd : 0; }
  }
  return { d, ux, uy };
}
function wallClearance(grid, x, y, reach) { return wallNearest(grid, x, y, reach).d; }
