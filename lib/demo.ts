...
// 했다. 그 손작업을 여기서 없앤다.
//
// **지어낸 데이터가 아니다.** 강원 속초시에서 6월에 음식·미식 축제를 새로
// 연다고 가정한 기획안이다. 지역·인구(8만)·접근성은 619건에 등록된 속초시
// 값이고(실향민문화축제 기록), 테마와 시기는 기획안이 고르는 값이다. 도면은
// lib/preset.ts 의 예시 배치를 그 지역 좌표와 줌 18 축척으로 깐 것이다.
//
// ── 횡성군에서 속초시로 옮긴 이유 (2026-09-09) ──
//
// 619건 배수를 lib/surge.ts 한 정의로 재계산하자(공사 KT 원본 2,658일) 횡성군 6월
// 음악·공연 조건의 쌍둥이 중앙 배수가 1.97 로 내려와 등급이 "주의"가 됐다. 시연은
// 심각 등급·감당 범위·도면 쏠림 셋이 한 화면에 다 뜨는 사례여야 하므로 619건 전수를
// (같은 지역값 + 다른 테마) 조건으로 다시 훑어 그 셋이 서는 14개 중 감당 범위 상한이
// 가장 큰 속초시 6월 음식·미식(중앙 2.05배, 상한 1.83배)을 골랐다.
...
// 뜨기 때문이다. 619건 전수로 조건을 훑어 골랐고, 그 셋이 계속 뜨는지는
// demo.test.ts 가 매번 다시 잰다. 619건이 다시 갱신되어 깨지면 고칠 것은 이 사례다.
...
/**
 * 지역값(인구·접근성)의 출처가 되는 619건의 기록(실향민문화축제, 속초 6월).
 * 조건 전체가 이 축제인 것은 **아니다** — 테마와 시기는 기획안이 고른다.
 * 이 축제는 감당 범위의 기준으로도 쓰인다. 테스트가 둘 다 대조한다.
 */
export const DEMO_REGION_FESTIVAL_ID = "2495286";
...
export const DEMO_ENTRY: Entry = {
  ...
  sigungu: "속초시",
  month: "6",
  // 음식·미식. 속초시 6월 기록(테마 3 지역 종합)과 다른 값이라 자기 자신을
  // 쌍둥이로 집지 않는다
  theme: "1",
  population: "8",
  accessibility: "3",
