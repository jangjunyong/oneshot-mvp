...
- 첫 화면(**기획안 넣기**)에 **기획서 PDF**를 올리면(지자체 양식 아니어도 됨) 예상 방문객·기간·지역·예산·지난 회차 개최 기간을 문서에서 옮겨 적어 판정 화면으로 갑니다. 함께 드린 **견본 기획안 PDF**(${JUDGE_GUIDE_URL}/견본_기획안_군포철쭉축제_2027.pdf)를 그대로 올리면 위 견본과 같은 판정이 섭니다. 모델은 숫자를 옮겨 적는 데만 쓰고 판정에는 쓰지 않습니다.
...
3. 배치를 고치려면 왼쪽 **편집** 에서 **선택·옮기기 / 부스 놓기 / 통로 그리기** 로 부스를 옮기거나 더하고 **도면 저장** 뒤 **처음부터 → 재생**. 견본 기획안 5쪽의 배치도가 이 도면과 같은 것입니다.
4. **가정 바꿔 보기 → 유입 시나리오** 에서 배수(×)·출입구별 유입 몫·1인 방문 부스 수·관람 체류(초)·난수 시드를 바꿉니다.
