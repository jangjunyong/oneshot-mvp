// 심사위원 시연 안내 — 문서 문자열을 코드에서 만든다 (2026-09-12 F, 사용자 지시 7. 2026-09-13 견본 제거·기획서 먼저로 고침).
//
...
4. **시뮬레이션 요약** 세 줄: 군포 실도면(부스 114·출입구 4)에서 개장 뒤 ${S.minutes}분을 미리 돌린 결과입니다. 최대 밀도 ${card.peak.density.toFixed(2)}명/㎡(${card.peak.where}). 미리 그린 도면이 군포뿐이라 군포 기획안에만 붙습니다.
...
군포 실도면 기대값(시드 ${S.seed} 고정, 재현 가능): ${S.minutes}분 재생에서 최대 밀도 ${card.peak.density.toFixed(2)}명/㎡ (${card.peak.where}), 3명/㎡ 이상 지속 ${card.hotspots.length}곳, 5명/㎡ 이상 ${card.secAbove5}초. 산출 ${base.builtAt}.
