...
1. 첫 화면에서 **견본으로 먼저 보기 → 군포철쭉축제 2027** 을 누릅니다.
...
6. 다른 견본: 판정 배너의 **화천산천어축제 →** (${(h.n ?? 0).toLocaleString("ko-KR")}명 발표치 → 상한 초과, 손익분기 회전율).

## 2. 자기 축제 넣어 보기

- 첫 화면에 **기획서 PDF**를 올리면(지자체 양식 아니어도 됨) 예상 방문객·기간·지역·예산을 문서에서 옮겨 적어 판정 화면으로 갑니다. 모델은 숫자를 옮겨 적는 데만 쓰고 판정에는 쓰지 않습니다.
