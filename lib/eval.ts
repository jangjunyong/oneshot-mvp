 ...
 *
 * **2026-08-31 재측정.** 임계값 0.24(DISTANCE_THRESHOLD) · 테마 가중치 0.15.
 *
 * 올라간 이유가 모델을 고쳐서가 아니다 — 619건 중 4건에 좌표가 비었거나
 * (19.69, 117.99) 기본값이 박혀 있었고, 지역 축이 그걸 2000km 대 거리로
 * 재고 있었다. 그 쓰레기를 걸러내니 판정이 좋아졌다. **없던 성능이 생긴 게
 * 아니라 있던 성능을 잘못 재고 있었던 것이다.**
 *
 * 이전 값 — 정밀도 0.6015 · 재현율 0.5479 · 리프트 2.550 · 밴드비율 0.7884
