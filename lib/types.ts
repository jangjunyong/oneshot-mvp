 ...
 *
 * 0.24 의 근거 — 619건 leave-one-out 실측에서 진짜 축제의 3번째 이웃 거리는
 * 최대 **0.2371**. 그 밖은 실측이 보증하지 않는 범위다.
 * match.test.ts 가 이 근거를 데이터로 다시 재서 지킨다(최대 이상 · 최대+0.01 이하).
 *
 * **2026-08-31 에 0.27 에서 내렸다.** 값이 바뀐 것이 아니라 자가 바뀌었다 —
 * 619건 중 4건에 좌표가 비었거나 (19.69, 117.99) 기본값이 박혀 있었고,
 * 지역 축이 그걸 2000km 대 거리로 재고 있었다. 그 쓰레기 거리가 3번째 이웃
 * 분포를 늘려 놓아 실측 최대가 0.2674 로 부풀어 있었다. 좌표를 걸러내니
 * 0.2371 로 좁아졌고, 임계값이 실측을 따라 내려온 것이다.
 */
export const DISTANCE_THRESHOLD = 0.24;
 ...
 */
export const DAILY_EXTRACT_LIMIT = 300; // 2026-09-18 사용자 결정: 심사 기간 비용은 감수, 심사단이 막히지 않게
 ...
 */
export const DAILY_EXTRACT_PER_CLIENT = 60; // 2026-09-18: 기관 NAT 뒤 심사위원 여럿이 한 IP 로 올 수 있다
