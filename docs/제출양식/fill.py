...
set_cell(
    ...
    "군포철쭉축제만 봐도 그렇습니다 — 시 발표 방문객은 2024년 360,370명에서 2025년 545,802명으로 1.51배가 됐는데"
    "(연합뉴스 2025-04-28), 같은 축제를 공사 KT 일별 실측으로 재면 최대일 외지인 배수는 1.47배 → 1.52배로 거의 그대로입니다. "
    "세 해 모두 축제 신호는 '뚜렷함'이라 측정이 못 잡은 것도 아닙니다. 두 숫자는 서로 다른 것을 잽니다 — 발표치는 절대 인원, "
    "배수는 평소 대비 상대값입니다. 그런데 예산 심의는 둘을 구분하지 않습니다. 이 서비스가 하는 일이 그 구분을 화면에서 강제하는 것입니다. "
...
set_cell(
    ...
    "6) 지자체 실무 채널 — 축제 담당 부서와 접촉해 추진결과 보고·차년도 기본계획의 실물 서식과 결재 리드타임(개최 약 7개월 전)을 확인했습니다. "
    "비공개 자료라 수치·원문은 인용하지 않고, 공개 출처로 재확인된 값만 화면에 씁니다(접촉 기록은 저장소 docs/interviews.md). "
    "다음 회차 기획 주기에 맞춰 1개 지자체 파일럿을 목표로 합니다.\n"
